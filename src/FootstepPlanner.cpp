...
MatrixN FootstepPlanner::updateFootsteps(int k, VectorN const& q,
                                         Vector6 const& b_v,
  ...
  if ((k % k_mpc_ == 0 && k != 0) || gait_->isNewPhase()) {
    ...
    // q_FK_.block(3, 0, 4, 1) =
    // pinocchio::SE3::Quaternion(pinocchio::rpy::rpyToMatrix(q(3, 0), q(4, 0),
    // 0.0)).coeffs(); q_FK_.tail(12) = q.tail(12);
    ...
    for (int i = 0; i < 4; i++) {
      if (currentGait(0, i) - previousGait_(0, i) >
        ...
        currentFootstep_.col(i) =
            Rz * (ftgPositions.col(i).head(3) -
                  (Vector3(q[0], q[1], 0.0) - Rz * b_vref.head(3) * dt_wbc));
        // currentFootstep_(2, i) = data_.oMf[foot_ids_[i]].translation()(2, 0);
        previousHeight_(0, i) = currentFootstep_(2, i);
        // std::cout << "= New contact at " <<
        // currentFootstep_.col(i).transpose() << " for foot " << i <<
        // std::endl;
      }
    }
  ...
  // Feet in contact with the ground are moving in base frame (they don't move
  // in world frame)
  double rotation_yaw =
      dt_wbc * b_vref(5);  // Rotation along Z for the last time step
  double c = std::cos(rotation_yaw);
  double s = std::sin(rotation_yaw);
  Rz.topLeftCorner<2, 2>() << c, s, -s, c;
  Vector2 dpos =
      dt_wbc *
      b_vref.head(2);  // Displacement along X and Y for the last time step
  for (int j = 0; j < 4; j++) {
    if (gait_->getCurrentGaitCoeff(0, j) == 1.0) {
      currentFootstep_.block(0, j, 2, 1) =
          Rz * (currentFootstep_.block(0, j, 2, 1) - dpos);
    }
  }
...
void FootstepPlanner::computeFootsteps(int k, Vector6 const& b_v,
                                       Vector6 const& b_vref) {
...
void FootstepPlanner::computeNextFootstep(int i, int j, Vector6 const& b_v,
                                          Vector6 const& b_vref) {
  nextFootstep_ = Matrix34::Zero();
  double t_stance = gait_->getPhaseDuration(i, j);

  // Disable heuristic terms if gait is going to switch to static so that feet
  // land at vertical of shoulders
  if (!gait_->getIsStatic()) {
    ...
    // Add feedback term
    nextFootstep_.col(j) +=
        params_->k_feedback * (b_v.head(3) - b_vref.head(3));
...
MatrixN FootstepPlanner::computeTargetFootstep(int k, Vector6 const& q,
                                               Vector6 const& b_v,
                                               Vector6 const& b_vref) {
  ...
  // Get o_targetFootstep_ in world frame from targetFootstep_ in horizontal
  // frame
  RPY_ = q.tail(3);
  ...
  Rz.topLeftCorner<2, 2>() << c, -s, s, c;
  for (int i = 0; i < 4; i++) {
    o_targetFootstep_.block(0, i, 2, 1) =
        Rz.topLeftCorner<2, 2>() * targetFootstep_.block(0, i, 2, 1) +
        q.head(2);
...
void FootstepPlanner::updateNewContact(Vector18 const& q) {
  ...
  q_FK_.block(3, 0, 4, 1) =
      pinocchio::SE3::Quaternion(
          pinocchio::rpy::rpyToMatrix(q(3, 0), q(4, 0), 0.0))
          .coeffs();
  ...
  for (int i = 0; i < 4; i++) {
    if (gait_->getCurrentGaitCoeff(0, i) == 1.0) {
      currentFootstep_.block(0, i, 2, 1) = pos_feet_.block(
          0, i, 2, 1);  // Get only x and y to let z = 0 for contacts
    }
  }
}
