...
void WbcWrapper::initialize(Params &params) {
  ...
  // Compute the upper triangular part of the joint space inertia matrix M by
  // using the Composite Rigid Body Algorithm Result is stored in data_.M
...
void WbcWrapper::compute(VectorN const &q, VectorN const &dq,
                         VectorN const &f_cmd, MatrixN const &contacts,
                         MatrixN const &pgoals, MatrixN const &vgoals,
                         MatrixN const &agoals, VectorN const &xgoals) {
  if (f_cmd.rows() != 12) {
  ...
  k_since_contact_ =
      k_since_contact_.cwiseProduct(contacts);  // Reset feet in swing phase
  ...
  invkin_->run(q_wbc_, dq_wbc_, contacts, pgoals.transpose(),
               vgoals.transpose(), agoals.transpose(), xgoals);
  ddq_cmd_ = invkin_->get_ddq_cmd();
  dq_cmd_ = invkin_->get_dq_cmd();
  q_cmd_ = invkin_->get_q_cmd();

  // TODO: Check if we can save time by switching MatrixXd to defined sized
  // vector since they are not called from python anymore

  // Retrieve feet jacobian  // TODO: Retrieve it in one go to avoid having Jc_
  // and Jc_u_
  for (int i = 0; i < 4; i++) {
    if (contacts(0, i)) {
  ...
  // Compute the inverse dynamics, aka the joint torques according to the
  // current state of the system, the desired joint accelerations and the
  // external forces, using the Recursive Newton Euler Algorithm. Result is
  // stored in data_.tau
  ...
  if (!enable_comp_forces_) {
    ...
    pinocchio::rnea(model_, data_, q_wbc_, dq_wbc_, ddq_cmd_);
    f_compensation = pseudoInverse(Jc_.transpose()) *
                     (data_.tau.head(6) - RNEA_without_joints + RNEA_NLE);
  }
  ...
  // std::cout << "ddq_cmd_bis " << std::endl << ddq_cmd_.transpose() <<
  // std::endl;

  // std::cout << "M : " << std::endl << data_.M.block(0, 0, 3, 18) <<
  // std::endl; std::cout << "ddq: " << std::endl << ddq_cmd_.transpose() <<
  // std::endl;

  /*
  std::cout << "-- BEFORE QP PROBLEM --" << std::endl;
  std::cout << "M ddq_u: " << std::endl << (data_.M.block(0, 0, 3, 6) *
  ddq_cmd_.head(6)).transpose() << std::endl; std::cout << "M ddq_a: " <<
  std::endl << (data_.M.block(0, 6, 3, 12) * ddq_cmd_.tail(12)).transpose() <<
  std::endl; pinocchio::rnea(model_, data_, q_wbc_, dq_wbc_,
  VectorN::Zero(model_.nv)); std::cout << "Non linear effects: " << std::endl <<
  data_.tau.head(6).transpose() << std::endl; std::cout << "JcT f_cmd + f_comp:
  " << std::endl << (Jc_.transpose() * (f_cmd + f_compensation)).transpose() <<
  std::endl; std::cout << "JcT f_comp: " << std::endl << (Jc_.transpose() *
  (f_compensation)).transpose() << std::endl;
  */

  // Solve the QP problem
  pinocchio::rnea(model_, data_, q_wbc_, dq_wbc_, ddq_cmd_);
  box_qp_->run(data_.M, Jc_, ddq_cmd_, f_cmd + f_compensation,
               data_.tau.head(6), k_since_contact_);
  ...
  Vector6 left = data_.M.block(0, 0, 6, 6) * box_qp_->get_ddq_res() -
                 Jc_.transpose() * box_qp_->get_f_res();
  Vector6 right =
      -data_.tau.head(6) + Jc_.transpose() * (f_cmd + f_compensation);
  Vector6 tmp_RNEA = data_.tau.head(6);

  // std::cout << "RNEA: " << std::endl << data_.tau.head(6).transpose() <<
  // std::endl; std::cout << "left: " << std::endl << left.transpose() <<
  // std::endl; std::cout << "right: " << std::endl << right.transpose() <<
  // std::endl; std::cout << "M: " << std::endl << data_.M.block(0, 0, 6, 6) <<
  // std::endl; std::cout << "JcT: " << std::endl << Jc_.transpose() <<
  // std::endl; std::cout << "M: " << std::endl << data_.M.block(0, 0, 3, 18) <<
  // std::endl;
  /*
  pinocchio::rnea(model_, data_, q_wbc_, dq_wbc_, VectorN::Zero(model_.nv));
  Vector6 tmp_NLE = data_.tau.head(6);

  // std::cout << "NLE: " << std::endl << data_.tau.head(6).transpose() <<
  std::endl;
  // std::cout << "M DDQ: " << std::endl << (tmp_RNEA -
  data_.tau.head(6)).transpose() << std::endl;
  // std::cout << "JcT f_cmd: " << std::endl << (Jc_.transpose() * (f_cmd +
  f_compensation)).transpose() << std::endl;
  // std::cout << "Gravity ?: " << std::endl <<  (data_.M * ddq_cmd_ -
  data_.tau.head(6)).transpose() << std::endl;
  ...
  */
  ...
  std::cout << "M DDQ Delta: " << std::endl << (data_.tau.head(6) -
  tmp_NLE).transpose() << std::endl; std::cout << "JcT f_cmd Delta: " <<
  std::endl << (Jc_.transpose() * f_with_delta_).transpose() << std::endl;*/
  ...
  // std::cout << " -- " << std::endl << invkin_->get_Jf().block(0, 6, 12, 12)
  // << std::endl << " -- " << std::endl << Jc_u_ << std::endl;
  ...
  // pinocchio::rnea(model_, data_, q_wbc_, dq_wbc_, ddq_with_delta_);
  // Vector6 tau_left = data_.tau.head(6); // data_.M * ddq_with_delta_.head(6)
  // - Jc_.transpose() * f_with_delta_; // Vector6 tau_right = Jc_.transpose() *
  // f_with_delta_; std::cout << "tau_left: " << std::endl <<
  // tau_left.transpose() << std::endl; std::cout << "tau_right: " << std::endl
  // << tau_right.transpose() << std::endl;
  ...
  pinocchio::rnea(model_, data_, q_wbc_, dq_wbc_, ddq_test);
  std::cout << "M DDQ Delta Bis: " << std::endl << (data_.tau.head(6) -
  tmp_NLE).transpose() << std::endl;*/

  /*
  std::cout << "-- AFTER QP PROBLEM --" << std::endl;
  std::cout << "M ddq_u: " << std::endl << (data_.M.block(0, 0, 3, 6) *
  ddq_with_delta_.head(6)).transpose() << std::endl; std::cout << "M ddq_a: " <<
  std::endl << (data_.M.block(0, 6, 3, 12) *
  ddq_with_delta_.tail(12)).transpose() << std::endl; pinocchio::rnea(model_,
  data_, q_wbc_, dq_wbc_, VectorN::Zero(model_.nv)); std::cout << "Non linear
  effects: " << std::endl << data_.tau.head(6).transpose() << std::endl;
  std::cout << "JcT f_cmd: " << std::endl << (Jc_.transpose() *
  f_with_delta_).transpose() << std::endl;

  std::cout << "LEFT " << (tmp_RNEA.head(3) + data_.M.block(0, 0, 3, 6) *
  box_qp_->get_ddq_res()).transpose() << std::endl;
  */
