...
// void Controller::compute(std::shared_ptr<odri_control_interface::Robot>
// robot) {
void Controller::compute(FakeRobot* robot) {
  ...
  estimator.run(
      gait.getCurrentGait(), footTrajectoryGenerator.getFootPosition(),
      robot->imu->GetLinearAcceleration(), robot->imu->GetGyroscope(),
      robot->imu->GetAttitudeEuler(), robot->joints->GetPositions(),
      robot->joints->GetVelocities(), Vector3::Zero(), Vector3::Zero());

  // Update state vectors of the robot (q and v) + transformation matrices
  // between world and horizontal frames
  estimator.updateReferenceState(joystick.getVRef());

  // Update gait
  gait.update(k, joystick.getJoystickCode());

  // Quantities go through a 1st order low pass filter with fc = 15 Hz (avoid
  // >25Hz foldback)
  q_filt_mpc.head(6) =
      filter_mpc_q.filter(estimator.getQReference().head(6), true);
  q_filt_mpc.tail(12) = estimator.getQReference().tail(12);
  h_v_filt_mpc = filter_mpc_v.filter(estimator.getHV().head(6), false);
  vref_filt_mpc =
      filter_mpc_vref.filter(estimator.getBaseVelRef().head(6), false);

  // Compute target footstep based on current and reference velocities
  o_targetFootstep = footstepPlanner.updateFootsteps(
      k, estimator.getQReference().head(18), estimator.getHVFiltered().head(6),
      estimator.getBaseVelRef().head(6),
      footTrajectoryGenerator.getFootPosition());

  // Run state planner (outputs the reference trajectory of the base)
  statePlanner.computeReferenceStates(k, q_filt_mpc.head(6), h_v_filt_mpc,
                                      vref_filt_mpc,
                                      footstepPlanner.getFootsteps().row(0));

  // Solve MPC problem once every k_mpc iterations of the main loop
  if (k % k_mpc == 0) {
    mpcWrapper.solve(k, statePlanner.getReferenceStates(),
                     footstepPlanner.getFootsteps(), gait.getCurrentGait());
  }
  ...
  if (!error && !joystick.getStop()) {
    ...
    // In static mode with L1 pressed, perform orientation control of the base
    // with joystick
    ...
    // If the four feet are in contact then we do not listen to MPC (default
    // contact forces instead)
    if (params_->DEMONSTRATION && gait.getIsStatic()) {
    ...
    q_wbc.tail(12) =
        wbcWrapper
            .get_qdes();  // with reference angular positions of previous loop

    // Update velocity vector for wbc
    dq_wbc.head(6) = estimator.getVEstimate().head(
        6);  // Velocities in base frame (not horizontal frame!)
    dq_wbc.tail(12) =
        wbcWrapper
            .get_vdes();  // with reference angular velocities of previous loop
    ...
    wbcWrapper.compute(q_wbc, dq_wbc, f_mpc, gait.getCurrentGait().row(0),
                       ...
                       hRb * estimator.getoRh().transpose() *
                           footTrajectoryGenerator.getFootVelocity(),
                       hRb * estimator.getoRh().transpose() *
...
void Controller::init_robot(Params& params) {
  ...
  for (int i = 0; i < 4; i++) {
    for (int j = 0; j < 3; j++) {
      ...
      params_->footsteps_under_shoulders[3 * i + j] =
...
void Controller::security_check() {
  ...
  if (((estimator.getQEstimate().tail(12).cwiseAbs()).array() >
       q_security_.array())
          .any()) {
    std::cout << "Position limit error "
              << ((estimator.getQEstimate().tail(12).cwiseAbs()).array() >
                  q_security_.array())
                     .transpose()
              << std::endl;
    error_flag = 1;
    ...
    std::cout
        ...
        << ((estimator.getVSecurity().cwiseAbs()).array() > 100.0).transpose()
        << std::endl;
    error_flag = 2;
  } else if (((tau_ff.cwiseAbs()).array() > 8.0).any()) {
    std::cout << "Feedforward limit error "
              << ((tau_ff.cwiseAbs()).array() > 8.0).transpose() << std::endl;
    error_flag = 3;
  } else {
    error_flag = 0;
  }
  ...
  // If Stop key of the joystick is pressed, set error flag to stop the
  // controller and switch to damping
  if (joystick.getStop()) {
    error = true;
    error_flag = -1;
  }

  // If something wrong happened in the controller we stick to a security
  // controller
  if (error) {
    // Quantities sent to the control board
