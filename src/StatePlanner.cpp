...
void StatePlanner::initialize(Params& params, Gait& gaitIn) {
  ...
  dt_vector_ =
      VectorN::LinSpaced(n_steps_, dt_, static_cast<double>(n_steps_) * dt_);
  gait_ = &gaitIn;
}
...
void StatePlanner::preJumpTrajectory(int i, double t_swing, int k) {
  ...
  // Third phase in chronological order: jump phase, no feet in contact,
  // ballistic motion

  // Second phase in chronological order: quickly raise the base to reach a
  // sufficient vertical velocity
  int j = 0;
  ...
  while (i >= 0 && (j * dt_ < (t_swing / 4 - 1e-5))) {
    ...
    referenceStates_(2, 1 + i) = h_ref_ + h_feet_mean_ +
                                 2.0 * g * std::pow(t_p, 2) * 0.5 -
                                 g * t_swing / 2 * t_p;
    // std::cout << "t_p: " << t_p << " | Pos: " << referenceStates_(2, 1 + i)
    // << " | Vel: " << referenceStates_(8, 1 + i) << std::endl;
    j++;
    i--;
  }
  for (int a = 0; a <= j; a++) {
    referenceStates_(6, 1 + i + a) =
        0.81 * 0.0 * (static_cast<double>(a) / static_cast<double>(j));
    if (i + a >= 0) {
      referenceStates_(0, 1 + i + a) =
          0.81 * 0.0 * dt_ + referenceStates_(0, i + a);
    }
  }
  ...
  while (i >= 0 && (j * dt_ <= t_swing)) {
    ...
    referenceStates_(2, 1 + i) = h_ref_ + h_feet_mean_ + A5 * std::pow(td, 5) +
                                 A4 * std::pow(td, 4) + A3 * std::pow(td, 3);
    referenceStates_(8, 1 + i) = 5 * A5 * std::pow(td, 4) +
                                 4 * A4 * std::pow(td, 3) +
                                 3 * A3 * std::pow(td, 2);
    j++;
    i--;
...
void StatePlanner::computeReferenceStates(int k, VectorN const& q,
                                          Vector6 const& v, Vector6 const& vref,
                                          MatrixN fsteps) {
  if (q.rows() != 6) {
  ...
  for (int i = 0; i < n_steps_; i++) {
    if (vref(5) != 0) {
      referenceStates_(0, 1 + i) =
          (vref(0) * std::sin(vref(5) * dt_vector_(i)) +
           vref(1) * (std::cos(vref(5) * dt_vector_(i)) - 1.0)) /
          vref(5);
      referenceStates_(1, 1 + i) =
          (vref(1) * std::sin(vref(5) * dt_vector_(i)) -
           vref(0) * (std::cos(vref(5) * dt_vector_(i)) - 1.0)) /
          vref(5);
    } else {
  ...
  for (int i = 0; i < n_steps_; i++) {
    if (false && gait.row(i).isZero())  // Enable for jumping
    {
      ...
      // Compute the reference trajectory of the CoM for time steps before the
      // jump phase preJumpTrajectory(i - 1, t_swing, k);
      ...
      while (i < n_steps_ && gait.row(i).isZero()) {
        if (i != 0) {
          referenceStates_(2, 1 + i) =
              h_ref_ + h_feet_mean_ - g * 0.5 * t_fly * (t_fly - t_swing);
          referenceStates_(8, 1 + i) = g * (t_swing * 0.5 - t_fly);
          referenceStates_(0, 1 + i) =
              0.81 * 0.0 * dt_ + referenceStates_(0, i);
          referenceStates_(6, 1 + i) = 0.81 * 0.0;
        } else {
          double t_p = t_fly + (k % 20) * dt_wbc_ - dt_;
          referenceStates_(2, 1 + i) =
              h_ref_ + h_feet_mean_ - g * 0.5 * t_p * (t_p - t_swing);
          referenceStates_(8, 1 + i) = g * (t_swing * 0.5 - t_p);
          referenceStates_(6, 1 + i) = 0.81 * 0.0;
        }
        // std::cout << t_swing << " | " << t_fly << " | " <<
        // referenceStates_(2, 1 + i) << std::endl; Pitch referenceStates_(4, 1
        // + i) = 0.5; referenceStates_(10, 1 + i) = 3.0;
        t_fly += dt_;
        i++;
      }
      i--;
