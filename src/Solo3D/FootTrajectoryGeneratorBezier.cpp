...
// Trajectory generator functions (output reference pos, vel and acc of feet in
// swing phase)
...
FootTrajectoryGeneratorBezier::FootTrajectoryGeneratorBezier()
  ...
  for (int i = 0; i < 4; i++) {
    ...
    pDefs[i].degree = 7;
    pDefs[i].flag = optimization::INIT_POS | optimization::END_POS |
                    optimization::INIT_VEL | optimization::END_VEL |
                    optimization::INIT_ACC | optimization::END_ACC;
    Vector6 vector = Vector6::Zero();
    problem_data_t pbData =
        optimization::setup_control_points<pointX_t, double, safe>(pDefs[0]);
    bezier_linear_variable_t* bez = pbData.bezier;

    bezier_t bezi =
        evaluateLinear<bezier_t, bezier_linear_variable_t>(*bez, vector);
...
void FootTrajectoryGeneratorBezier::initialize(Params& params, Gait& gaitIn,
  ...
  targetFootstep_ << Eigen::Map<Eigen::VectorXd, Eigen::Unaligned>(
      params.footsteps_init.data(), params.footsteps_init.size());
  position_ << Eigen::Map<Eigen::VectorXd, Eigen::Unaligned>(
      params.footsteps_init.data(), params.footsteps_init.size());
  gait_ = &gaitIn;
...
void FootTrajectoryGeneratorBezier::updatePolyCoeff_XY(
    int const& i_foot, Vector3 const& x_init, Vector3 const& v_init,
    Vector3 const& a_init, Vector3 const& x_target, double const& t0,
    double const& t1) {
  double x0 = x_init(0);
  double y0 = x_init(1);
  double dx0 = v_init(0);
  ...
  Ax(5, i_foot) = (ddx0 * std::pow(t, 2) - 2 * ddx0 * t * d - 6 * dx0 * t +
                   ddx0 * std::pow(d, 2) + 6 * dx0 * d + 12 * x0 - 12 * x1) /
                  (2 * std::pow((t - d), 2) *
                   (std::pow(t, 3) - 3 * std::pow(t, 2) * d +
                    3 * t * std::pow(d, 2) - std::pow(d, 3)));

  Ax(4, i_foot) =
      (30 * t * x1 - 30 * t * x0 - 30 * d * x0 + 30 * d * x1 -
       2 * std::pow(t, 3) * ddx0 - 3 * std::pow(d, 3) * ddx0 +
       14 * std::pow(t, 2) * dx0 - 16 * std::pow(d, 2) * dx0 + 2 * t * d * dx0 +
       4 * t * std::pow(d, 2) * ddx0 + std::pow(t, 2) * d * ddx0) /
      (2 * std::pow((t - d), 2) *
       (std::pow(t, 3) - 3 * std::pow(t, 2) * d + 3 * t * std::pow(d, 2) -
        std::pow(d, 3)));
  Ax(3, i_foot) =
      (std::pow(t, 4) * ddx0 + 3 * std::pow(d, 4) * ddx0 -
       8 * std::pow(t, 3) * dx0 + 12 * std::pow(d, 3) * dx0 +
       20 * std::pow(t, 2) * x0 - 20 * std::pow(t, 2) * x1 +
       20 * std::pow(d, 2) * x0 - 20 * std::pow(d, 2) * x1 + 80 * t * d * x0 -
       80 * t * d * x1 + 4 * std::pow(t, 3) * d * ddx0 +
       28 * t * std::pow(d, 2) * dx0 - 32 * std::pow(t, 2) * d * dx0 -
       8 * std::pow(t, 2) * std::pow(d, 2) * ddx0) /
      (2 * std::pow((t - d), 2) *
       (std::pow(t, 3) - 3 * std::pow(t, 2) * d + 3 * t * std::pow(d, 2) -
        std::pow(d, 3)));
  Ax(2, i_foot) =
      -(std::pow(d, 5) * ddx0 + 4 * t * std::pow(d, 4) * ddx0 +
        3 * std::pow(t, 4) * d * ddx0 + 36 * t * std::pow(d, 3) * dx0 -
        24 * std::pow(t, 3) * d * dx0 + 60 * t * std::pow(d, 2) * x0 +
        60 * std::pow(t, 2) * d * x0 - 60 * t * std::pow(d, 2) * x1 -
        60 * std::pow(t, 2) * d * x1 -
        8 * std::pow(t, 2) * std::pow(d, 3) * ddx0 -
        12 * std::pow(t, 2) * std::pow(d, 2) * dx0) /
      (2 * (std::pow(t, 2) - 2 * t * d + std::pow(d, 2)) *
       (std::pow(t, 3) - 3 * std::pow(t, 2) * d + 3 * t * std::pow(d, 2) -
        std::pow(d, 3)));
  Ax(1, i_foot) =
      -(2 * std::pow(d, 5) * dx0 - 2 * t * std::pow(d, 5) * ddx0 -
        10 * t * std::pow(d, 4) * dx0 + std::pow(t, 2) * std::pow(d, 4) * ddx0 +
        4 * std::pow(t, 3) * std::pow(d, 3) * ddx0 -
        3 * std::pow(t, 4) * std::pow(d, 2) * ddx0 -
        16 * std::pow(t, 2) * std::pow(d, 3) * dx0 +
        24 * std::pow(t, 3) * std::pow(d, 2) * dx0 -
        60 * std::pow(t, 2) * std::pow(d, 2) * x0 +
        60 * std::pow(t, 2) * std::pow(d, 2) * x1) /
      (2 * std::pow((t - d), 2) *
       (std::pow(t, 3) - 3 * std::pow(t, 2) * d + 3 * t * std::pow(d, 2) -
        std::pow(d, 3)));
  Ax(0, i_foot) =
      (2 * x1 * std::pow(t, 5) - ddx0 * std::pow(t, 4) * std::pow(d, 3) -
       10 * x1 * std::pow(t, 4) * d +
       2 * ddx0 * std::pow(t, 3) * std::pow(d, 4) +
       8 * dx0 * std::pow(t, 3) * std::pow(d, 3) +
       20 * x1 * std::pow(t, 3) * std::pow(d, 2) -
       ddx0 * std::pow(t, 2) * std::pow(d, 5) -
       10 * dx0 * std::pow(t, 2) * std::pow(d, 4) -
       20 * x0 * std::pow(t, 2) * std::pow(d, 3) +
       2 * dx0 * t * std::pow(d, 5) + 10 * x0 * t * std::pow(d, 4) -
       2 * x0 * std::pow(d, 5)) /
      (2 * (std::pow(t, 2) - 2 * t * d + std::pow(d, 2)) *
       (std::pow(t, 3) - 3 * std::pow(t, 2) * d + 3 * t * std::pow(d, 2) -
        std::pow(d, 3)));

  Ay(5, i_foot) = (ddy0 * std::pow(t, 2) - 2 * ddy0 * t * d - 6 * dy0 * t +
                   ddy0 * std::pow(d, 2) + 6 * dy0 * d + 12 * y0 - 12 * y1) /
                  (2 * std::pow((t - d), 2) *
                   (std::pow(t, 3) - 3 * std::pow(t, 2) * d +
                    3 * t * std::pow(d, 2) - std::pow(d, 3)));
  Ay(4, i_foot) =
      (30 * t * y1 - 30 * t * y0 - 30 * d * y0 + 30 * d * y1 -
       2 * std::pow(t, 3) * ddy0 - 3 * std::pow(d, 3) * ddy0 +
       14 * std::pow(t, 2) * dy0 - 16 * std::pow(d, 2) * dy0 + 2 * t * d * dy0 +
       4 * t * std::pow(d, 2) * ddy0 + std::pow(t, 2) * d * ddy0) /
      (2 * std::pow((t - d), 2) *
       (std::pow(t, 3) - 3 * std::pow(t, 2) * d + 3 * t * std::pow(d, 2) -
        std::pow(d, 3)));
  Ay(3, i_foot) =
      (std::pow(t, 4) * ddy0 + 3 * std::pow(d, 4) * ddy0 -
       8 * std::pow(t, 3) * dy0 + 12 * std::pow(d, 3) * dy0 +
       20 * std::pow(t, 2) * y0 - 20 * std::pow(t, 2) * y1 +
       20 * std::pow(d, 2) * y0 - 20 * std::pow(d, 2) * y1 + 80 * t * d * y0 -
       80 * t * d * y1 + 4 * std::pow(t, 3) * d * ddy0 +
       28 * t * std::pow(d, 2) * dy0 - 32 * std::pow(t, 2) * d * dy0 -
       8 * std::pow(t, 2) * std::pow(d, 2) * ddy0) /
      (2 * std::pow((t - d), 2) *
       (std::pow(t, 3) - 3 * std::pow(t, 2) * d + 3 * t * std::pow(d, 2) -
        std::pow(d, 3)));
  Ay(2, i_foot) =
      -(std::pow(d, 5) * ddy0 + 4 * t * std::pow(d, 4) * ddy0 +
        3 * std::pow(t, 4) * d * ddy0 + 36 * t * std::pow(d, 3) * dy0 -
        24 * std::pow(t, 3) * d * dy0 + 60 * t * std::pow(d, 2) * y0 +
        60 * std::pow(t, 2) * d * y0 - 60 * t * std::pow(d, 2) * y1 -
        60 * std::pow(t, 2) * d * y1 -
        8 * std::pow(t, 2) * std::pow(d, 3) * ddy0 -
        12 * std::pow(t, 2) * std::pow(d, 2) * dy0) /
      (2 * (std::pow(t, 2) - 2 * t * d + std::pow(d, 2)) *
       (std::pow(t, 3) - 3 * std::pow(t, 2) * d + 3 * t * std::pow(d, 2) -
        std::pow(d, 3)));
  Ay(1, i_foot) =
      -(2 * std::pow(d, 5) * dy0 - 2 * t * std::pow(d, 5) * ddy0 -
        10 * t * std::pow(d, 4) * dy0 + std::pow(t, 2) * std::pow(d, 4) * ddy0 +
        4 * std::pow(t, 3) * std::pow(d, 3) * ddy0 -
        3 * std::pow(t, 4) * std::pow(d, 2) * ddy0 -
        16 * std::pow(t, 2) * std::pow(d, 3) * dy0 +
        24 * std::pow(t, 3) * std::pow(d, 2) * dy0 -
        60 * std::pow(t, 2) * std::pow(d, 2) * y0 +
        60 * std::pow(t, 2) * std::pow(d, 2) * y1) /
      (2 * std::pow((t - d), 2) *
       (std::pow(t, 3) - 3 * std::pow(t, 2) * d + 3 * t * std::pow(d, 2) -
        std::pow(d, 3)));
  Ay(0, i_foot) =
      (2 * y1 * std::pow(t, 5) - ddy0 * std::pow(t, 4) * std::pow(d, 3) -
       10 * y1 * std::pow(t, 4) * d +
       2 * ddy0 * std::pow(t, 3) * std::pow(d, 4) +
       8 * dy0 * std::pow(t, 3) * std::pow(d, 3) +
       20 * y1 * std::pow(t, 3) * std::pow(d, 2) -
       ddy0 * std::pow(t, 2) * std::pow(d, 5) -
       10 * dy0 * std::pow(t, 2) * std::pow(d, 4) -
       20 * y0 * std::pow(t, 2) * std::pow(d, 3) +
       2 * dy0 * t * std::pow(d, 5) + 10 * y0 * t * std::pow(d, 4) -
       2 * y0 * std::pow(d, 5)) /
      (2 * (std::pow(t, 2) - 2 * t * d + std::pow(d, 2)) *
       (std::pow(t, 3) - 3 * std::pow(t, 2) * d + 3 * t * std::pow(d, 2) -
        std::pow(d, 3)));
...
void FootTrajectoryGeneratorBezier::updatePolyCoeff_Z(int const& i_foot,
                                                      Vector3 const& x_init,
                                                      Vector3 const& x_target,
                                                      double const& t1,
                                                      double const& h) {
  double z0 = x_init(2);
...
Vector3 FootTrajectoryGeneratorBezier::evaluateBezier(int const& i_foot,
                                                      int const& indice,
                                                      double const& t) {
...
Vector3 FootTrajectoryGeneratorBezier::evaluatePoly(int const& i_foot,
                                                    int const& indice,
                                                    double const& t) {
  Vector3 vector = Vector3::Zero();
  if (indice == 0) {
    double x = Ax(0, i_foot) + Ax(1, i_foot) * t +
               Ax(2, i_foot) * std::pow(t, 2) + Ax(3, i_foot) * std::pow(t, 3) +
               Ax(4, i_foot) * std::pow(t, 4) + Ax(5, i_foot) * std::pow(t, 5);
    double y = Ay(0, i_foot) + Ay(1, i_foot) * t +
               Ay(2, i_foot) * std::pow(t, 2) + Ay(3, i_foot) * std::pow(t, 3) +
               Ay(4, i_foot) * std::pow(t, 4) + Ay(5, i_foot) * std::pow(t, 5);
    double z = Az(0, i_foot) + Az(1, i_foot) * t +
               Az(2, i_foot) * std::pow(t, 2) + Az(3, i_foot) * std::pow(t, 3) +
               Az(4, i_foot) * std::pow(t, 4) + Az(5, i_foot) * std::pow(t, 5) +
               Az(6, i_foot) * std::pow(t, 6);
    vector << x, y, z;
  }

  if (indice == 1) {
    double vx = Ax(1, i_foot) + 2 * Ax(2, i_foot) * t +
                3 * Ax(3, i_foot) * std::pow(t, 2) +
                4 * Ax(4, i_foot) * std::pow(t, 3) +
                5 * Ax(5, i_foot) * std::pow(t, 4);
    double vy = Ay(1, i_foot) + 2 * Ay(2, i_foot) * t +
                3 * Ay(3, i_foot) * std::pow(t, 2) +
                4 * Ay(4, i_foot) * std::pow(t, 3) +
                5 * Ay(5, i_foot) * std::pow(t, 4);
    double vz = Az(1, i_foot) + 2 * Az(2, i_foot) * t +
                3 * Az(3, i_foot) * std::pow(t, 2) +
                4 * Az(4, i_foot) * std::pow(t, 3) +
                5 * Az(5, i_foot) * std::pow(t, 4) +
                6 * Az(6, i_foot) * std::pow(t, 5);
    vector << vx, vy, vz;
  }

  if (indice == 2) {
    double ax = 2 * Ax(2, i_foot) + 6 * Ax(3, i_foot) * t +
                12 * Ax(4, i_foot) * std::pow(t, 2) +
                20 * Ax(5, i_foot) * std::pow(t, 3);
    double ay = 2 * Ay(2, i_foot) + 6 * Ay(3, i_foot) * t +
                12 * Ay(4, i_foot) * std::pow(t, 2) +
                20 * Ay(5, i_foot) * std::pow(t, 3);
    double az = 2 * Az(2, i_foot) + 6 * Az(3, i_foot) * t +
                12 * Az(4, i_foot) * std::pow(t, 2) +
                20 * Az(5, i_foot) * std::pow(t, 3) +
                30 * Az(6, i_foot) * std::pow(t, 4);
    vector << ax, ay, az;
  }
...
void FootTrajectoryGeneratorBezier::updateFootPosition(
    int const& k, int const& i_foot, Vector3 const& targetFootstep) {
  ...
  if (t0 < t1 - lockTime_) {
    ...
    if (t0s[i_foot] < 10e-4 || k == 0) {
      ...
      updatePolyCoeff_Z(i_foot, position_.col(i_foot), targetFootstep, t1,
                        h + height_);
      // Initale velocity and acceleration nulle
      updatePolyCoeff_XY(i_foot, position_.col(i_foot), Vector3::Zero(),
                         Vector3::Zero(), targetFootstep, t0, t1);

      // Update initial conditions of the Problem Definition
      Vector3 vector;
      vector << position_(0, i_foot), position_(1, i_foot),
          evaluatePoly(i_foot, 0, t0)(2);
      ...
      if ((newSurface_[i_foot].getHeight(targetFootstep.head(2)) -
        ...
        for (int l = 0; l < nb_vert; l++) {
          ...
          if (doIntersect_segment(P1, P2, Q1, Q2)) {
            ...
            if (ineq_[i_foot].transpose() * position_.col(i_foot) >
                ineq_vector_[i_foot]) {
            ...
            x_margin_[i_foot] = std::max(
                std::min(x_margin_max_,
                         std::abs(intersectionPoint_[0] - P1[0]) - 0.001),
                0.);
          }
        }
      } else {
      ...
      updatePolyCoeff_XY(i_foot, position_.col(i_foot), velocity_.col(i_foot),
                         acceleration_.col(i_foot), targetFootstep, t0, t1);
      // Update initial conditions of the Problem Definition
    ...
    pDefs[i_foot].flag = optimization::INIT_POS | optimization::END_POS |
                         optimization::INIT_VEL | optimization::END_VEL |
                         optimization::INIT_ACC | optimization::END_ACC;
    ...
    // problemDefinition
    problem_data_t pbData =
        optimization::setup_control_points<pointX_t, double, safe>(
            pDefs[i_foot]);
    ...
    double t_margin =
        t_margin_ *
        t1;  // 10% around the limit point !inferior to 1/nb point in linspace

    // No surface switch or already overpass the critical point
    if (!(ineq_[i_foot].isZero()) and
        ((x_t[2] < targetFootstep(2)) or
         (t0s[i_foot] < t_stop[i_foot] + t_margin)) and
        x_margin_[i_foot] != 0.) {
      int nb_vert = newSurface_[i_foot].vertices_.rows();
      MatrixN vert = newSurface_[i_foot].vertices_;
      ...
      double z_margin =
          targetFootstep(2) * z_margin_;  // 10% around the limit height
      double t_s;
      double zt;
      ...
      for (int its = 0; its < N_samples_ineq; its++) {
        ...
        if (t0 + (t1 - t0) * t_s < t_stop[i_foot] + t_margin) {
          ...
          h_(its) = -ineq_[i_foot].transpose() * linear_var_.c() +
                    ineq_vector_[i_foot] - (x_margin_[i_foot]);
        } else {
          G_.row(its).setZero();
          h_(its) = 0.;
        }
    ...
    for (int j = 0; j < N_samples; j++) {
      ...
      q_ += linear_var.B().transpose() *
            (linear_var.c() - evaluatePoly(i_foot, 0, t0 + (t1 - t0) * t_b_));
    }
    ...
    fitBeziers[i_foot] =
        evaluateLinear<bezier_t, bezier_linear_variable_t>(*linear_bezier, x);
  ...
  double t_b =
      std::min((ev - t0_bezier[i_foot]) / (t1 - t0_bezier[i_foot]), 1.);
  ...
  if (t0 < 0.0 || t0 > t1)  // Just vertical motion
  {
    ...
    if (useBezier) {
      ...
      acceleration_.col(i_foot) =
          fitBeziers[i_foot].derivate(t_b, 2) / std::pow(delta_t, 2);
    } else {
...
void FootTrajectoryGeneratorBezier::update(
    int k, MatrixN const& targetFootstep, SurfaceVector const& surfacesSelected,
    VectorN const& q) {
  if ((k % k_mpc) == 0) {
...
bool FootTrajectoryGeneratorBezier::doIntersect_segment(Vector2 const& p1,
                                                        Vector2 const& q1,
                                                        Vector2 const& p2,
                                                        Vector2 const& q2) {
...
bool FootTrajectoryGeneratorBezier::onSegment(Vector2 const& p,
                                              Vector2 const& q,
                                              Vector2 const& r) {
  if ((q[0] <= std::max(p[0], r[0])) and (q[0] >= std::min(p[0], r[0])) and
      (q[1] <= std::max(p[1], r[1])) and (q[1] >= std::min(p[1], r[1]))) {
    return true;
  }
  return false;
}
...
int FootTrajectoryGeneratorBezier::orientation(Vector2 const& p,
                                               Vector2 const& q,
                                               Vector2 const& r) {
  double val =
...
void FootTrajectoryGeneratorBezier::get_intersect_segment(Vector2 a1,
                                                          Vector2 a2,
                                                          Vector2 b1,
                                                          Vector2 b2) {
  Vector3 cross_l1;
