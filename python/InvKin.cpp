...
struct InvKinVisitor : public bp::def_visitor<InvKinVisitor<InvKin>> {
  ...
  void visit(PyClassInvKin& cl) const {
    cl.def(bp::init<>(bp::arg(""), "Default constructor."))
        ...
        .def("initialize", &InvKin::initialize, bp::args("params"),
             "Initialize InvKin from Python.\n")

        .def("get_q_step", &InvKin::get_q_step,
             "Get position step of inverse kinematics.\n")
        ...
        .def("get_foot_id", &InvKin::get_foot_id, bp::args("i"),
             "Get food frame id.\n")

        // Run InvKin from Python
        .def("run", &InvKin::run,
             bp::args("contacts", "pgoals", "vgoals", "agoals", "x_cmd"),
             "Run InvKin from Python.\n");
  }
