...
struct MpcWrapperVisitor
    : public bp::def_visitor<MpcWrapperVisitor<MpcWrapper>> {
  ...
  void visit(PyClassMpcWrapper& cl) const {
    cl.def(bp::init<>(bp::arg(""), "Default constructor."))
        ...
        .def("solve", &MpcWrapper::solve,
             bp::args("k", "xref_in", "fsteps_in", "gait_in"),
        ...
        .def("get_temporary_result", &MpcWrapper::get_temporary_result,
             bp::args("gait"),
             "Use a temporary result if contact status changes between two "
             "calls of the MPC.\n")
        .def("stop_parallel_loop", &MpcWrapper::stop_parallel_loop,
             "Stop the parallel loop.\n")
        .def_readonly("t_mpc_solving_duration",
                      &MpcWrapper::get_t_mpc_solving_duration,
                      "Solving time of latest MPC iteration.\n");
  }

  static void expose() {
    bp::class_<MpcWrapper>("MpcWrapper", bp::no_init)
        .def(MpcWrapperVisitor<MpcWrapper>());
  }
};
