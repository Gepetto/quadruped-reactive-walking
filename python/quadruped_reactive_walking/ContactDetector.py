...
class ContactDetector():
    ...
    def __init__(self, params):
        ...
        # IDs of feet frames
        self.foot_ids = [self.r.model.getFrameId('FL_FOOT'),
    ...
    def getContactForces(self, device):
        ...
        contactPoints_FL = pyb.getContactPoints(device.pyb_sim.robotId, device.pyb_sim.planeId, linkIndexA=3)  # Front left  foot
        contactPoints_FR = pyb.getContactPoints(device.pyb_sim.robotId, device.pyb_sim.planeId, linkIndexA=7)  # Front right foot
        contactPoints_HL = pyb.getContactPoints(device.pyb_sim.robotId, device.pyb_sim.planeId, linkIndexA=11)  # Hind left  foot
        contactPoints_HR = pyb.getContactPoints(device.pyb_sim.robotId, device.pyb_sim.planeId, linkIndexA=15)  # Hind right foot
        ...
        for contact in contactPoints:
            if not isinstance(contact, int):  # type(contact) != type(0):
                for i_direction in range(0, 3):
                    forces[3 * cpt + i_direction] = (contact[9] * contact[7][i_direction] + contact[10] *
    ...
    def run(self, k, gait, q, dq, tau, device, qdes):
        ...
        if self.is_simulation and k > 0:
            ...
            ctc_FL = pyb.getContactPoints(device.pyb_sim.robotId, device.pyb_sim.planeId, linkIndexA=3)  # Front left foot
            ctc_FR = pyb.getContactPoints(device.pyb_sim.robotId, device.pyb_sim.planeId, linkIndexA=7)  # Front right foot
            ctc_HL = pyb.getContactPoints(device.pyb_sim.robotId, device.pyb_sim.planeId, linkIndexA=11)  # Hind left foot
            ctc_HR = pyb.getContactPoints(device.pyb_sim.robotId, device.pyb_sim.planeId, linkIndexA=15)  # Hind right foot

            ctc_FL_block = pyb.getContactPoints(device.pyb_sim.robotId, 26, linkIndexA=3)  # Front left foot
            ctc_FR_block = pyb.getContactPoints(device.pyb_sim.robotId, 26, linkIndexA=7)  # Front right foot
            ctc_HL_block = pyb.getContactPoints(device.pyb_sim.robotId, 26, linkIndexA=11)  # Hind left foot
            ctc_HR_block = pyb.getContactPoints(device.pyb_sim.robotId, 26, linkIndexA=15)  # Hind right foot
            a = np.array([len(ctc_FL), len(ctc_FR), len(ctc_HL), len(ctc_HR)]) > 0
            b = np.array([len(ctc_FL_block), len(ctc_FR_block), len(ctc_HL_block), len(ctc_HR_block)]) > 0
            self.log_ctc_truth[k, :] = np.logical_or(a, b).astype(int)

            # Log ground truth for feet velocities
            linkStates = pyb.getLinkStates(device.pyb_sim.robotId, [3, 7, 11, 15], computeLinkVelocity=1)
            for i in range(4):
                self.log_v_truth[k, i] = (linkStates[i][6][0]**2 + linkStates[i][6][1]**2 + linkStates[i][6][2]**2)**0.5
        ...
        # Feet jacobians
        for i in range(4):
            self.Jf[(3*i):(3*(i+1)), :] = pin.getFrameJacobian(self.r.model, self.r.data,
                                                               self.foot_ids[i], pin.LOCAL_WORLD_ALIGNED)[:3, 6:]
        ...
        for i in range(4):
            v = pin.getFrameVelocity(self.r.model, self.r.data, self.foot_ids[i], pin.LOCAL_WORLD_ALIGNED)
        ...
        for i in range(4):
            ...
            # Feet velocity
            v_norm = (self.log_v[k, 3*i]**2 + self.log_v[k, 3*i+1]**2 + self.log_v[k, 3*i+2]**2)**0.5
            self.P_v[k, i] = 1 - self.P(v_norm, 0.18, 0.074) # 0.121, 0.0411

            # Contact plan
            P_0to1 = 0.5 * (1.0 + erf((self.log_ctc[k, i] - 0.0) / (80.0 * (2**0.5))))
            sw = np.any([np.logical_and(self.log_ctc[k, i] >= 0, self.log_gait[k, i] == 1),
                         np.logical_and(self.log_ctc[k, i] < 0, self.log_gait[k, i] == 0),
                         self.k_since_late[i] > 0]).astype(int)
            self.P_cc[k, i] = sw * P_0to1

            # Product of probabilities for final estimation
            self.P_tot[k, i] = np.max([self.P_f[k, i] * self.P_v[k, i]]) # * self.P_cc[k, i]
