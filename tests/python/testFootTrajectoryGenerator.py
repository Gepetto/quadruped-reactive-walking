...
class TestFootTrajectoryGenerator(unittest.TestCase):
    ...
    def test_non_moving(self):
        ...
        for k in range(500):
            ...
            o_tgt = self.ftg.getTargetPosition()
    ...
    def test_non_moving_sin(self):
        ...
        # Moving reference
        t = np.round(np.linspace(0, (self.params.N_SIMULATION-1) * self.params.dt_wbc, self.params.N_SIMULATION), 3)
        ...
        ref = np.array(self.params.footsteps_under_shoulders.tolist()).reshape((3, 4), order='F')
        ref_lock = ref.copy()
        t_lock = np.round(self.params.T_gait * 0.5 - self.params.lock_time - self.params.vert_time - self.params.dt_wbc, 3)
        ...
        for k in range(self.params.N_SIMULATION-1):
            ...
            # Target in ideal world
            o_tgt = self.ftg.getTargetPosition()
            o_pos = self.ftg.getFootPosition()
            o_vel = self.ftg.getFootVelocity()
            o_acc = self.ftg.getFootAcceleration()
            ...
            for j in range(4):
                ...
                if cgait[0, j] == 1:
                    self.assertTrue(np.allclose(ref_lock[:, j], o_tgt[:, j]), 'o_tgt is OK')
                else:
                    ...
                    if tmod == t_lock:
                        ...
                        self.assertTrue(np.allclose(ref[:, j] + osc[:, k], o_tgt[:, j]), 'o_tgt is OK')

                # Check 3D output of generator (pos, vel, acc)
                if cgait[0, j] == 1:
                    self.assertTrue(np.allclose(o_tgt[:, j], o_pos[:, j]), 'o_pos is OK')
                    self.assertTrue(o_pos[2, j] < 1e-8, 'o_pos is OK')
                    self.assertTrue(np.allclose(np.zeros(3), o_vel[:, j]), 'o_vel is OK')
                    self.assertTrue(np.allclose(np.zeros(3), o_acc[:, j]), 'o_acc is OK')
                else:
                    tmod = np.round(t[k] % (self.params.T_gait * 0.5), 3)
                    if (tmod < self.params.vert_time) or (tmod >= np.round(self.params.T_gait * 0.5 - self.params.vert_time, 3)):
                        self.assertTrue(np.allclose(ref_lock[:2, j], o_pos[:2, j]), 'o_pos is OK')
                        self.assertTrue(np.allclose(np.zeros(2), o_vel[:2, j]), 'o_vel is OK')
                        self.assertTrue(np.allclose(np.zeros(2), o_acc[:2, j]), 'o_acc is OK')
                    """
